...
SELECT hotel_no, AVG(booking_count) AS avg_bookings FROM (
 SELECT hotel_no, COUNT(*) AS booking_count FROM booking WHERE MONTH(date_from) = 8
    GROUP BY hotel_no) AS booking_counts
GROUP BY hotel_no;

--4. What is the most commonly booked room type for each hotel in London?

--5. What is the lost income from unoccupied rooms at each hotel today?
SELECT h.hotel_no, SUM(r.price) AS lost_income FROM room r
JOIN hotel h ON r.hotel_no = h.hotel_no
LEFT JOIN booking b ON r.room_no = b.room_no 
WHERE b.room_no IS NULL OR b.date_to < CAST(GETDATE() AS DATE)
GROUP BY h.hotel_no;
